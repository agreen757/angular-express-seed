//***************************************
/*
ADD EXPORTS SHIT TO THIS

WE NEED TO ROCK THIS THE SAME WAY WE ARE DOING THE MONGO MODULE

BASICALLY WRITE THE FILE AND SEND THE READY SHIT BACK TO THE APP.JS TO BE SENT TO THE CONTROLLER TO THE HTML AND SHOW THE DOWNLOAD OPTION

THE DOWNLOAD WILL BE A LINK TO A RES.DOWNLOAD IN THE APP.JS
*/
...
exports.soundcloud = function(metadata,files,cb){
    ...
    //*************THIS NEEDS TO BE THE METADATA CSV THAT WE SENT FROM THE CONTROLLER 
    //var sourceName = process.argv[2]+".csv"
    var sourceName = metadata.path;
    //**************

    var foo = metadata.name.split('.');
    var fileName = foo[0]+".xml";
    ...
    fs.appendFileSync(fileName, '<MessageThreadId>'+date+'IND</MessageThreadId>')
            ...
            if(element.resourceType == "Image"){
                ...
                async.series([
                    ...
                    function(cb){
                        ...
                        fs.appendFileSync(fileName, '<FileName>'+element.nameFile+'</FileName>\n');
                        fs.appendFileSync(fileName, '<FilePath>'+element.nameFile+'</FilePath>\n');
